...
def config(tmp_path: Path, data_root: Path) -> PipelineConfig:
    ...
    # BUG FIX (monorepo relocation): see test_schema_validator.py's
    # validator fixture for the identical fix and full explanation —
    # configs/ lives under data-forge/, not alongside tests/ anymore.
    _configs_root = Path(__file__).parent.parent.parent / "data-forge" / "configs"
    config.prompts_dir = _configs_root / "prompts"
    config.schemas_dir = _configs_root / "schemas"
    return config
