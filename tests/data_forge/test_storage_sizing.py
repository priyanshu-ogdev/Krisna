...
class TestPreferencePairRelevantCount:
    ...
    def test_hpdv2_ranked_list_mode_also_counted_as_preference_pairs(self):
        """Regression guard: HPDv2 uses a dedicated download_mode
        ("hpdv2_ranked_list") because its real schema (a variable-length
        ranked list per prompt) doesn't fit the generic "preference_pair"
        two-image-table path — see fetcher.py::_fetch_hpdv2_ranked_pairs.
        An earlier revision's storage accounting only recognized the
        literal string "preference_pair", which would have left HPDv2
        counted as 60,000 ordinary image records in
        storage_relevant_record_count() (wrong estimate keys, wrong
        storage projection) while contributing nothing to
        preference_pair_relevant_count() at all (its real cost invisible)
        — the exact same class of bug as literal-string-vs-set drift
        found in s01_6_preference_pairs.py's source filter.
        """
        spec = _spec(
            expected_record_count=798_000,
            fetch_config={"download_mode": "hpdv2_ranked_list", "sample_size": 60_000},
        )
        assert spec.preference_pair_relevant_count() == 60_000
        assert spec.storage_relevant_record_count() == 0
