...
class DatasetSpec:
    ...
    # `preference_pair`/`hpdv2_ranked_list` (Pick-a-Pic v2/HPDv2/
    # DesignSense-10k/DesignPref — written directly to preference_pairs/,
    # see fetcher.py::_fetch_huggingface_preference_pairs and
    # ::_fetch_hpdv2_ranked_pairs), and `eval_reference` (TASTE/
    # PartiPrompts — written directly to heldout/external_eval/, never
    # touches training data at all). These consume ~0 of the per-record
    # image-storage budget in pipeline.yaml's `per_record_estimates`,
    # which assumes a raw+scrubbed+latent+vq+control+metadata image
    # record — projecting full storage for them would double-count
    # storage that either doesn't exist (annotations are tiny compared to
    # per_record_estimates) or was already counted under the dataset they
    # join onto (rico_core/rico_semantic), or is tracked separately under
    # its own preference-pair/eval storage estimate instead.
    #
    # BUG FIX: this set and PREFERENCE_PAIR_DOWNLOAD_MODES below used to
    # be defined independently in two places (here and in
    # stages/s01_6_preference_pairs.py's source-selection filter). When
    # HPDv2 was moved from "preference_pair" to its own
    # "hpdv2_ranked_list" mode (its real schema doesn't fit the generic
    # two-image-table path — see fetcher.py), only one of the two sets
    # was updated at first, which would have both (a) silently excluded
    # HPDv2's pairs from dedup/blur/safety post-processing and (b)
    # mis-projected its storage as if it were 60,000 ordinary image
    # records instead of 60,000 preference pairs. Centralized here so
    ...
    _ZERO_IMAGE_STORAGE_MODES: ClassVar[frozenset[str]] = frozenset(
        {"caption_join", "eval_reference"} | PREFERENCE_PAIR_DOWNLOAD_MODES
    )
    ...
    def preference_pair_relevant_count(self) -> int:
        ...
        storage_relevant_record_count() for preference-pair sources
        (any mode in PREFERENCE_PAIR_DOWNLOAD_MODES), which are excluded
        from that method (see _ZERO_IMAGE_STORAGE_MODES) because a pair
        costs a different, larger amount of storage than a single image
        record (two source images + two latents, tracked via
        ...
        """
        if self.fetch_config.get("download_mode") not in self.PREFERENCE_PAIR_DOWNLOAD_MODES:
            return 0
        sample_size = self.fetch_config.get("sample_size")
        if sample_size is not None:
            return min(self.expected_record_count, int(sample_size))
