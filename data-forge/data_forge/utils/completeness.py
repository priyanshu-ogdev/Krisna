...
Stage 8 (Tri-Path Encoding) produces a different artifact set depending on
a record's domain: `vq_tokens` (Branch 3, sketch-tier training data) is
only computed for `ui_first` records — see s08_encoding.py's domain gate.
A `general_design` record correctly having three artifacts instead of
four is NOT an incompleteness — it's the intended, resource-efficient
behavior. Any code checking "did this record encode successfully" needs
to know that distinction rather than applying one blanket "all four
artifacts present" rule, which would incorrectly flag every properly-
processed general_design record as broken.
"""
...
# Every record, regardless of domain, needs these three.
_REQUIRED_ALWAYS = frozenset({"z_image_latent", "qwen_image_latent", "control_map"})
