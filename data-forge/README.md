...
- **The Planner had no training data path at all.** Now moot — the planner ships **frozen** (RAG over UICrit's real critique text + constrained JSON decoding), per the no-RLHF-loop PRD revision. See `s01_5_uicrit_join.py` (still active) and `s12_model_data_export.py`'s `planner_rag_corpus/` export. `s01_6_planner_synthesis` and the Glaive/xLAM format-teacher datasets it used have been removed.
- **Qwen-Image-Edit-2511 only ever saw single finished images**, not the (rough sketch, instruction, polished target) triples its actual edit-conditioned task needs. Also now moot — Qwen-Image-Edit-2511 ships **frozen** (zero-shot in-context-learning edit conditioning + SDEdit-style partial denoising at inference time), so it never needs paired training data at all. `s07_5_edit_pairs` and the MagicBrush/InstructPix2Pix ingestion have been removed; the only training data-forge still fine-tunes for the polish tier is Z-Image-Turbo's base latents plus Diffusion-DPO alignment (`s08_5_dpo_encoding.py`, `pickapic_v2`/`hpdv2`/`designsense_10k`/`designpref` in `datasets.yaml`).
